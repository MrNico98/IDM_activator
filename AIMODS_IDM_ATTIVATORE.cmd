...
echo:
if not exist "%IDMan%" (
call :_color %Red% "IDM [Internet Download Manager] non è installato."
echo Puoi scaricarlo da AIMODS Telegram
goto done
)
...
if not defined _fileexist call :_color %Red% "Errore: Unable to download files with IDM."
...
echo %line%
echo:
call :_color %Green% "Attivazione completata con successo."
echo:
call :_color %Gray% "Se viene visualizzata una schermata seriale falsa, eseguire nuovamente l'opzione di attivazione e non utilizzare l'opzione di ripristino."
echo:
...
if defined terminal (
call :_color %_Yellow% "Premi 0 per tornare indientro..."
choice /c 0 /n
) else (
call :_color %_Yellow% "Premi un tasto per chiudere..."
pause %nul1%
)
exit
...
if defined terminal (
echo Premi 0 per tornare indientro...
choice /c 0 /n
) else (
